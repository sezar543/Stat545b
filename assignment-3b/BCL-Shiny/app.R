...
ui <- fluidPage(
  ...
  titlePanel(windowTitle = "BC Liquor Store prices",
             title = 
               div("BC Liquor Store prices",
                   img(
                     src = "nhl.png",
                     height = 300,
                     width = 300,
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      tabsetPanel(type = "tabs",
                  tabPanel("Result", h3(textOutput("summaryText")),
                           downloadButton("download", "Download results")
                  ),
                  tabPanel("Plot", 
                           colourInput("col", "Select colour of Beer", "purple"),
                           colourInput("col2", "Select colour of wine", "pink"),
                           plotOutput("plot") 
                  ),
                  #tableOutput("prices")
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    ggplot(prices(), aes(Alcohol_Content, fill = Type)) +
      ...
      scale_fill_manual(values=c(input$col, input$col2)) +
